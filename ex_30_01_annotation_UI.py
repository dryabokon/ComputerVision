...
# ---------------------------------------------------------------------------------------------------------------------
from utils_primities import Lines, Players, Polygons
# ---------------------------------------------------------------------------------------------------------------------
import warnings
...
class markup:
    ...
    def __init__(self, folder_in):
        ...
        if self.display_primitives=='L':
            self.Lines = Lines()
            self.Lines.read_from_file(self.folder_in + 'lines.txt',self.dict_filenames)
        ...
        if self.display_primitives=='B':
            self.Boxes = Players(self.folder_in + 'boxes.txt',self.dict_filenames)
...
def process_mouse():
    ...
    if g_mouse_event_code == 'LBUTTONUP':
        if M.display_primitives=='L':
            M.add_line()
            M.Lines.save_to_file(M.folder_in + 'lines.txt', M.filenames)

        elif M.display_primitives == 'B':
            M.add_box()
            M.Boxes.save_to_file(M.folder_in + 'boxes.txt', M.filenames)
    ...
    if g_mouse_event_code == 'MOUSEWHEEL_SHF_FW':
        if M.display_primitives == 'L':M.current_frame_ID = M.get_next_frame_by_marked_line(look_forward=False)
        if M.display_primitives == 'B':M.current_frame_ID = M.get_next_frame_by_marked_box(look_forward=False)
        g_mouse_event_code = None
        M.refresh_image()
        return 1
    ...
    if g_mouse_event_code == 'MOUSEWHEEL_SHF_BK':
        if M.display_primitives == 'L':M.current_frame_ID = M.get_next_frame_by_marked_line(look_forward=True)
        if M.display_primitives == 'B':M.current_frame_ID = M.get_next_frame_by_marked_box(look_forward=True)
        g_mouse_event_code = None
        M.refresh_image()
        return 1
...
def process_key(key):
    ...
    if key == 26 and M.display_primitives in ['L','B']:
        ...
        M.refresh_image()
        if M.Lines is not None:M.Lines.save_to_file(M.folder_in + 'lines.txt', M.filenames)
        if M.Boxes is not None:M.Boxes.save_to_file(M.folder_in + 'boxes.txt', M.filenames)
        return 1
...
def application_loop_lines():
    ...
    return
# ---------------------------------------------------------------------------------------------------------------------
#folder_in = './data/ex_bin5/'
folder_in = './data/ex_soccer/'
M = markup(folder_in)
# ---------------------------------------------------------------------------------------------------------------------
