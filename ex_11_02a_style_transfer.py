...
num_colors = 3  # RGB
# ----------------------------------------------------------------------------------------------------------------------
ref_img = cv2.imread(target_mask_path).astype(numpy.float)
img_nrows, img_ncols = ref_img.shape[:2]
# ----------------------------------------------------------------------------------------------------------------------
total_variation_weight = 10.
style_weight = 0.1
content_weight = 0.5 if use_content_img else 0
# ----------------------------------------------------------------------------------------------------------------------
...
def load_mask_labels():
    ...
    if K.image_data_format() == 'channels_first':
        mask_vecs = numpy.vstack([style_mask_img.reshape((3, -1)).T,
                                  target_mask_img.reshape((3, -1)).T])
    else:
        mask_vecs = numpy.vstack([style_mask_img.reshape((-1, 3)),
    ...
    style_mask = numpy.stack([style_mask_label == r for r in range(num_labels)],
                             axis=stack_axis)
    target_mask = numpy.stack([target_mask_label == r for r in range(num_labels)],
                              axis=stack_axis)

    return (numpy.expand_dims(style_mask, axis=0),
            numpy.expand_dims(target_mask, axis=0))
# ----------------------------------------------------------------------------------------------------------------------
def gram_matrix(x):
    assert K.ndim(x) == 3
...
if __name__ == '__main__':
    ...
    if use_content_img:
        content_image = K.variable(preprocess_image(target_img_path))
    else:
    ...
    if K.image_data_format() == 'channels_first':
        x = numpy.random.uniform(0, 255, (1, 3, img_nrows, img_ncols)) - 128.
    else:
        x = numpy.random.uniform(0, 255, (1, img_nrows, img_ncols, 3)) - 128.

    for i in range(50):
        start_time = time.time()
        x, min_val, info = fmin_l_bfgs_b(evaluator.loss, x.flatten(),fprime=evaluator.grads, maxfun=20)

        cv2.imwrite(target_img_prefix + '_at_iteration_%02d.png' % i,deprocess_image(x.copy()))
